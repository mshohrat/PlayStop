...
public class MKLoader extends View implements InvalidateListener {
  ...
  private void initialize(Context context, AttributeSet attrs, int defStyleAttr) {
    TypedArray typedArray = context.obtainStyledAttributes(attrs, com.tuyenmonkey.mkloader.R.styleable.MKLoader);
    int loaderType = typedArray.getInt(com.tuyenmonkey.mkloader.R.styleable.MKLoader_mk_type,-1);

    loaderView = LoaderGenerator.generateLoaderView(loaderType);
    loaderView.setColor(typedArray.getColor(com.tuyenmonkey.mkloader.R.styleable.MKLoader_mk_color, Color.parseColor("#ffffff")));

    typedArray.recycle();
  }

  public void setColorResource(@ColorRes int colorResId) {
    if(getContext() == null) return;
    setColor(ContextCompat.getColor(getContext(),colorResId));
  }

  public void setColor(@ColorInt int color) {
    if(loaderView == null) return;
