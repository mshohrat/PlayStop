package cab.snapp.extensions.calendar
...
class JalaliCalendar : Calendar {
    ...
    constructor(
        ...
        var yearMonthDate = YearMonthDate(
                calendar[YEAR],
                calendar[MONTH],
                calendar[DATE]
        )
        yearMonthDate = gregorianToJalali(yearMonthDate)
        set(yearMonthDate.year, yearMonthDate.month, yearMonthDate.date)
        complete()
    }
    ...
    internal constructor(
        ...
        cal = GregorianCalendar(
                yearMonthDate.year, yearMonthDate.month, yearMonthDate.date, hourOfDay,
                minute, second
        )
    ...
    override fun computeTime() {
        if (!isTimeSet && !isTimeSeted) {
            val cal =
                    GregorianCalendar.getInstance(Companion.timeZone)
            ...
            if (internalGet(HOUR_OF_DAY) in 12..23) {
                super.set(AM_PM, PM)
            ...
            cal[yearMonthDate.year, yearMonthDate.month, yearMonthDate.date, internalGet(HOUR_OF_DAY), internalGet(
                    MINUTE
            )] =
                    internalGet(SECOND)
            time = cal.timeInMillis
            ...
            super.set(
                    ZONE_OFFSET,
                    Companion.timeZone.rawOffset
            )
            super.set(
                    DST_OFFSET,
                    Companion.timeZone.dstSavings
            )
            val yearMonthDate = jalaliToGregorian(
                    YearMonthDate(
                            internalGet(YEAR),
            ...
            cal!![yearMonthDate.year, yearMonthDate.month, yearMonthDate.date, internalGet(
                    HOUR_OF_DAY
            ), internalGet(MINUTE)] =
                    internalGet(SECOND)
            time = cal!!.timeInMillis
    ...
    override fun set(field: Int, value: Int) {
        when (field) {
            ...
            HOUR_OF_DAY, HOUR, MINUTE, SECOND, MILLISECOND, ZONE_OFFSET, DST_OFFSET -> {
                if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(
                                HOUR
                        ) && isSet(HOUR_OF_DAY) &&
                        isSet(MINUTE) && isSet(SECOND) && isSet(
                                MILLISECOND
                        )
                ) {
                    cal = GregorianCalendar()
                    ...
                    super.set(DATE, yearMonthDate.date)
                    super.set(
                            HOUR_OF_DAY,
                            cal!![HOUR_OF_DAY]
                    )
        ...
        when (field) {
            MONTH -> {
                ...
                if (get(DAY_OF_MONTH) > jalaliDaysInMonth[amount % 12]
                    ...
                    super.set(
                            DAY_OF_MONTH,
                            jalaliDaysInMonth[amount % 12]
                    )
                    if (get(MONTH) == 11 && isLeepYear(
                                    get(
                                            YEAR
                                    )
                            )
            ...
            YEAR -> {
                ...
                if (get(DAY_OF_MONTH) == 30 && get(MONTH) == 11 && !isLeepYear(
                                get(YEAR)
                        )
                ) {
    ...
    override fun roll(field: Int, amount: Int) {
        ...
        when (field) {
            ...
            HOUR_OF_DAY -> {
                run {
                    fields[HOUR_OF_DAY] =
                            (internalGet(HOUR_OF_DAY) + amount) % 24
                ...
                run {
                    ...
                    if (internalGet(MONTH) == 11 && isLeepYear(
                                    internalGet(YEAR)
                            )
                    ) {
                        monthLen = 30
            ...
            DAY_OF_YEAR -> {
                val unit =
                        if (isLeepYear(internalGet(YEAR))) 366 else 365
                ...
                super.set(
                        DAY_OF_MONTH,
                        jalaliDaysInMonth[internalGet(MONTH)] - (temp - dayOfYear)
                )
            }
            DAY_OF_WEEK -> {
                var index = amount % 7
    ...
    companion object {
        ...
        fun gregorianToJalali(gregorian: YearMonthDate): YearMonthDate {
            ...
            gregorian.date = gregorian.date - 1
            gregorianDayNo =
                    (365 * gregorian.year + floor((gregorian.year + 3) / 4.toDouble()).toInt()
                            - floor((gregorian.year + 99) / 100.toDouble()).toInt()
                            + floor((gregorian.year + 399) / 400.toDouble()).toInt())
            i = 0
        ...
        fun jalaliToGregorian(jalali: YearMonthDate): YearMonthDate {
            ...
            jalali.date = jalali.date - 1
            jalaliDayNo =
                    365 * jalali.year + (jalali.year / 33) * 8 + floor((jalali.year % 33 + 3) / 4.toDouble())
                            .toInt()
            i = 0
            ...
            gregorianYear += 4 * floor(gregorianDayNo / 1461.toDouble())
        ...
        fun weekOfYear(dayOfYear: Int, year: Int): Int {
            ...
            dayOfYear = floor(dayOfYear / 7.toDouble()).toInt()
            return dayOfYear + 1
        }
